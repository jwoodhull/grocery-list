...
Stage 3 (grocery list review/edit + HTML artifact) is complete and has been verified with a full manual end-to-end run (recipe reuse, paste, and manual/no-recipe entry; review edits; categorization; finalize; artifact generation). `.claude/skills/plan-week/SKILL.md` runs the conversational "plan dinners → get a grocery list" flow end to end, calling `load-recipes.ts`/`save-recipe.ts` (recipe reuse and persistence), `build-grocery-list.ts` (coverage + exact-match merge, now including each recipe's `mealType`), and `finalize-list.ts` (shape validation + five-category grouping) before composing the final HTML artifact via the Artifact tool. `finalize-list.ts` accepts its payload via `--dataFile` (a scratch JSON file) rather than an inline `--data` string, so item/recipe names with apostrophes survive shell quoting intact. Shared validation predicates live in `src/lib/validation.ts` (used by both `recipes.ts` and `finalize-list.ts`, so the two can't silently drift on rules like ingredient amounts) and shared CLI plumbing lives in `src/cli/util.ts` (`runCli`, `parseJsonArg`, `parseNumberArg`). Still no pantry, repeating items, scaling, or unit conversion — every grocery item is categorized by Claude's judgment (no `category` comes from `repeating.csv` yet) and no item is ever marked as a repeat. The full staged build plan and current stage is tracked in `spec/tech-spec.md` §6; work proceeds stage by stage from there.
