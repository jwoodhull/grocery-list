...
**Stage 3 — Grocery list review/edit + HTML artifact — ✅ Complete**

- Build: mostly `SKILL.md` instructions for conversational add/remove and artifact generation (per `artifact-design` conventions); a small `finalize-list.ts` (`validateFinalizedList` shape validation + `groupByCategory`) plus its `finalize-list.ts` CLI wrapper, which accepts its payload via `--dataFile <path>` (a scratch JSON file written by the skill) rather than an inline `--data '<json>'` shell argument, so recipe/item names containing an apostrophe don't break shell quoting. The artifact groups the grocery list into five fixed categories, always rendered in this order: **Dairy, Meat, Fresh Fruit & Veg, Dry Pantry Items, Household Goods**. Items sourced from `repeating.csv`/`due-items.ts` use their stored `category` field directly (§3); items sourced from recipe ingredients have no category field in the schema, so Claude assigns one by judgment at composition time (consistent with §4's "HTML artifact composition stays conversational" rule — tagging every recipe ingredient with a category up front isn't worth the added friction). `groupByCategory` rejects (rather than silently drops) an item whose category isn't one of the five. Items that came from a repeating/cadence item rather than a recipe are visually marked as repeats (e.g. a small badge or icon) so the user can see at a glance which items are recurring purchases vs. one-off for this week's recipes. `build-grocery-list.ts`'s `recipes` output now also includes `mealType`, so it doubles as `finalize-list.ts`'s `mealPlan` input. Shared validation predicates (`isNonEmptyString`/`isFiniteNumber`) live in `src/lib/validation.ts`, used by both `recipes.ts` and `finalize-list.ts` — `groceryList[].amount` allows zero (e.g. "to taste"), matching `recipes.ts`'s own ingredient-amount rule. Shared CLI plumbing (`runCli`, `parseJsonArg`, `parseNumberArg`) lives in `src/cli/util.ts`, used by all four CLI scripts.
- Tests: unit tests for `finalize-list.ts`'s `validateFinalizedList`/`groupByCategory` (`src/lib/finalize-list.test.ts`), `src/cli/util.test.ts` for the shared CLI helpers, and added edge-case coverage in `recipes.test.ts` (non-finite servings/amounts, zero ingredient amount).
- Verify: remove/add an item conversationally, confirm the final artifact reflects edits and renders the meal plan + grocery list clearly, grouped into the five fixed categories in order, with repeat items visibly marked. In early runs of this stage (before Stage 6 wires in repeating items), the repeat-marking simply has nothing to mark yet — no separate code path needed once Stage 6 lands. Manually verified end to end: recipe reuse, paste (including an apostrophe in the name), and manual/no-recipe entry; running coverage past full coverage; a review-step add/remove edit; categorization; finalize via `--dataFile`; and a generated HTML artifact with correct five-category grouping and an omitted empty Household category.
...
- Build: `csv.ts` (generic read/write), `repeatingStore.ts`, `cadence.ts` (`isDue`/`computeDueItems`), seed `data/repeating.csv` with a starter list; extend `build-grocery-list.ts` to merge due items. SKILL.md: step 4 (build the final grocery list) also calls the new `due-items.ts` and merges its due repeating/cadence items into the presented list. Note: `mergeIngredients` (§4) only dedups by normalized name+unit, with no notion of provenance — this stage must decide explicitly how a due repeating item that collides by name with a recipe-derived ingredient is resolved (which `category`/`isRepeat` wins, whether amounts still sum) rather than leaving it to whichever code path runs last.
...
**Stage 11 — Schema-validation refactor (conditional, not scheduled)**

- Trigger: revisit this once Stages 6-8 (`repeating.csv`/pantry validation) land and the codebase is hand-rolling a third or fourth structural validator (`recipes.ts`'s `validateNewRecipeInput`, `finalize-list.ts`'s `validateFinalizedList`/`groupByCategory`, plus whatever `pantry.ts`/`repeatingStore.ts` need). Not scheduled on its own — only worth doing if the validated-shape count has actually grown, not preemptively.
- Why: hand-rolled validators keep a `types/index.ts` interface and a separate `validate*` function in sync by hand for each shape; Stage 3 already produced a real bug from exactly that drift (`recipes.ts` allowed a zero ingredient amount, `finalize-list.ts`'s validator initially didn't). A schema library (e.g. Zod) derives the TypeScript type from the schema instead of maintaining both by hand, removing that failure mode structurally.
- Build: introduce the library as a dependency; replace `src/lib/validation.ts`'s `isNonEmptyString`/`isFiniteNumber` plus the `recipes.ts`/`finalize-list.ts` validators with schemas that both validate and produce the corresponding `types/index.ts` types, removing the now-redundant hand-written interfaces.
- Tests: existing validator test suites (`recipes.test.ts`, `finalize-list.test.ts`) should pass largely unchanged against the new schema-backed validators — a green suite through the swap is the main regression check.
- Exit criteria: one consistent validation approach across all schemas, less hand-maintained validation code, no behavior change versus the hand-rolled version it replaces.
